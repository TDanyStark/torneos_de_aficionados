...
{
    private const STATUSES = ['registration', 'in_progress', 'finished', 'archived'];
    ...
    protected function handle(): Response
    {
        ...
        // Slug can be changed/regenerated by the organizer (e.g. if spammed).
        // It is always sanitized to a URL-safe form and made unique.
        if (array_key_exists('slug', $body)) {
            $rawSlug = trim((string) $body['slug']);
            if ($rawSlug === '') {
                $errors['slug'] = 'El enlace del torneo no puede estar vacío.';
            } else {
                $sanitized = Slug::make($rawSlug);
                if ($sanitized !== $tournament->slug) {
                    $data['slug'] = $this->uniqueSlug($sanitized, $tournament->id);
                }
            }
        }
        ...
        if (array_key_exists('is_public', $body)) {
            $data['is_public'] = !empty($body['is_public']);
        }
    ...
    /**
     * Ensures a slug is unique, appending a numeric suffix on collision. The
     * tournament's own current slug never collides (the caller only changes the
     * slug when it differs), so any match here belongs to a different row.
     */
    private function uniqueSlug(string $base, int $currentId): string
    {
        $slug = $base !== '' ? $base : 'torneo';
        $candidate = $slug;
        $suffix = 1;

        while ($this->tournaments->slugExists($candidate)) {
            $existing = $this->tournaments->findBySlug($candidate);
