...
/**
 * POST /api/v1/tournaments/{id}/registration-photo  (auth, code-gated, multipart)
 *
 * Public self-registration helper: a delegate uploads a PLAYER photo (camera or
 * gallery) before submitting the registration. Authorization is by the
 * registration_code (multipart field "registration_code"), NOT a pre-existing
 * role, mirroring CreateRegistrationAction. The image is compressed +
 * center-cropped to EXACTLY 398x398 and stored under
 * api/public/uploads/players/. Returns the URL only; it is persisted later when
 * the registration is submitted.
 *
 * Response 200: { "photo_url": "/uploads/players/<hex>.jpg" }
 */
final class UploadRegistrationPhotoAction extends ApiAction
{
    ...
    protected function handle(): Response
    {
        ...
        return $this->responder->success($this->response, ['photo_url' => $photoUrl]);
    }
